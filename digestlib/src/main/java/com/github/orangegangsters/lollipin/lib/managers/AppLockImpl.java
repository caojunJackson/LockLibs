...
public class AppLockImpl<T extends AppLockActivity> extends AppLock implements LifeCycleInterface {
     ...
     * 建立超时锁
     */
    private static final String IS_ENABLE_TIMEOUT_LOCK = "IS_ENABLE_TIMEOUT_LOCK";
    ...
    public void disableAndRemoveConfiguration() {
        ...
        mSharedPreferences.edit().remove(PASSWORD_PREFERENCE_KEY)
                ...
                .remove(IS_ENABLE_TIMEOUT_LOCK) //移除添加的是否加入超时解锁设置
                .apply();
    }
     ...
     */
    @Override
    public void setShouldAddTimeOutLock(boolean isTimeoutLock) {
        SharedPreferences.Editor editor = mSharedPreferences.edit();
        editor.putBoolean(IS_ENABLE_TIMEOUT_LOCK, isTimeoutLock);
        editor.apply();
    }

    @Override
    public boolean getShouldAddTimeOutLock() {
        return mSharedPreferences.getBoolean(IS_ENABLE_TIMEOUT_LOCK, false);
    }
